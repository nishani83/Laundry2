...
class order {
    ...
    public function viewPendingOrdersBySchedule($scheduleID) {
        $con = $GLOBALS['con'];
        $sql = "select * from schedule_weborder s left join orders on s.orderID=orders.orderID left join customer on customer.customerID=orders.customerID WHERE s.scheduleID='$scheduleID' and orders.orderStatus='assigned'";
        //weborder.weborderID,customer.name,weborder.pickupDate,orders.amount,orders.orderStatus
        $result = $con->query($sql);
        return $result;
    }
