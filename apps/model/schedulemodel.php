...
WHERE s.scheduleID='$scheduleID' and o.orderStatus='ondelivery'";
    ...
    public function viewAllPickUp() {
        $con = $GLOBALS['con'];
        //sql query
        $sql = "SELECT * FROM schedule_weborder s
    ...
    public function todayPickupCountByCity() {
        $con = $GLOBALS['con'];
        //sql query
        $sql = "SELECT count(orders.orderID) as count,customer.city FROM orders left join customer on customer.customerID=orders.customerID left join weborder on weborder.weborderID=orders.orderID where orders.orderStatus='pending' AND weborder.pickupDate=CURDATE() group by customer.city ";
        //Execute a query
        $result = $con->query($sql);
        return $result;
    }

    public function todayDeliveryCountByCity() {
        $con = $GLOBALS['con'];
        //sql query
        $sql = "SELECT count(orders.orderID) as count,customer.city FROM orders left join customer on customer.customerID=orders.customerID left join weborder on weborder.weborderID=orders.orderID where orders.orderStatus='delivered' AND weborder.pickupDate=CURDATE() group by customer.city ";
        //Execute a query
        $result = $con->query($sql);
        return $result;
    }
